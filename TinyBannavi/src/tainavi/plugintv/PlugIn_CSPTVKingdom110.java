...
public class PlugIn_CSPTVKingdom110 extends PlugIn_TVPTVKingdom implements TVProgram,Cloneable {
	...
	private void _loadProgram(Center cr, boolean force, int counter, int counterMax) {
		//
		try {
			// 局リストの追加
			...
			if (force == true ||
				...
				//
				String url = String.format("http://tv.so-net.ne.jp/chart/%s/%s.action?parentId=9999&childId=%s&id=%s&head=%04d%02d%02d0500&span=24",
	...
	public void loadCenter(String code, boolean force) {
		...
		if (getCSType().equals(csCode)) {
			_loadCenter(newcrlist,code,getCSType(),"http://tv.so-net.ne.jp/chart/cs/skylist.action");
		}
		else {
			_loadCenter(newcrlist,code,getCSType(),"http://tv.so-net.ne.jp/chart/cs110/e2list.action");
		}
